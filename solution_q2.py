...
def q2_DFS(initialState):
    ...
    while stack:
        ...
        # print("state", state)
        topRowNums = [state[0], state[1], state[2]]
        for i in topRowNums:
            if (i == "_"):
                topRowNums[topRowNums.index(i)] = 0
            else:
                topRowNums[topRowNums.index(i)] = int(i)
        # print("nodeCount", nodeCount)
        if (sum(topRowNums)) == goalState:
            return path
        ...
        for move in moves:
            if ((move == "D") & (emptySpotIndex not in (6, 7, 8))):
                ...
                if (tuple(tempState) not in visited):
                    queue.append((tempState, "".join(path) + (movingNumber + "U")))
            ...
            if ((move == "U") & (emptySpotIndex not in (0, 1, 2))):
                ...
                if (tuple(tempState) not in visited):
                    queue.append((tempState, "".join(path) + (movingNumber + "D")))
            ...
            if ((move == "R") & (emptySpotIndex not in (2, 5, 8))):                
                ...
                if (tuple(tempState) not in visited):
                    queue.append((tempState, "".join(path) + (movingNumber + "L")))
            ...
            if ((move == "L") & (emptySpotIndex not in (0, 3, 6))):
                ...
                if (tuple(tempState) not in visited):
                    queue.append((tempState, "".join(path) + (movingNumber + "R")))
                    
    return None

def q2_UCS(initialState):
...
def q2_Astar_Manhattan(initialState):
...
def q2_Astar_Euclidian(initialState):
